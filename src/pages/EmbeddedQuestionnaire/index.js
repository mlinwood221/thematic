...
import { Row, Col, Button, Placeholder, SelectPicker, TagGroup } from 'rsuite';
import ClientAPI from './../../api/client';

const themes = () => THEMES;
const activities = () => ACTIVITIES;

class EmbeddedQuestionnaire extends Component {
    state = {
        currentStep: 0,
    ...
    componentDidMount() {
        this.fetchCurrent();
        // let adviser = advisers[this.props.match.params.id]
    ...
    fetchCurrent = async () => {
        try {
            ...
            let res = await ClientAPI.embeddedCurrent(this.props.match.params.id);
            this.setState({
                adviser: res.data
            });
        } catch (error) {
    ...
    createClient = async () => {
        ...
        try {
            let res = await ClientAPI.newProspect(req, this.props.match.params.id);
        } catch (error) {
        ...
        switch (idx) {
                    ...
                    <Row style={{ textAlign: "center" }}>
                        <form>
                            <h2 className="sentence">I would like to retire in <input
                                ...
                                onChange={this.handleInfoChange}
                                name="retirement"
                                className="sentence-input" type="number" /> years</h2>
                        </form>
                    </Row>
                    <Row style={{ marginTop: "20px" }}>
                        <Col xs={8} xsOffset={8} md={8} mdOffset={8} style={{ textAlign: "center", marginTop: "20px" }}>
            ...
            case 7:
                return <div className="step-8">
                    ...
                    <Row style={{ marginTop: "20px" }}>
                        <Col xs={8} xsOffset={8} md={8} mdOffset={8} style={{ textAlign: "center", marginTop: "20px" }}>
                            <Button disabled={!this.state.clientInfo.first_name || !this.state.clientInfo.last_name || !this.state.clientInfo.email || !this.state.clientInfo.email.match(/.+@.+\../)} onClick={this.nextStep} className="questionnaire-button">
                                Continue
                                    </Button>
                            <br></br>
                            <br></br>
    ...
    render() {
        return (
            <div style={{ backgroundColor: "#F5F7FD", height: "100vh" }}>
                <Row>
                    <Col xs={20} xsOffset={2} md={16} mdOffset={4}>
                        ...
                        <div className="card Questionnaire-Flow" style={{ marginTop: "70px" }}>
                            <Row>
                                <Col md={6}
